...
- `--force` は **state もキャッシュも無視して infer.py を再実行**し、全セリフを作り直す
  （キャッシュに壊れた/空の音声が入っている場合の作り直しにも使う）。`--force` は「検証エラーを
  隠してクラッシュさせる」ためのものではなく、ユーザー向けエラー表示を保ったまま再生成を強制する。
- infer.py の出力が異常に小さい（壊れ/空）場合は、その行を失敗として記録し、**キャッシュしない**。
